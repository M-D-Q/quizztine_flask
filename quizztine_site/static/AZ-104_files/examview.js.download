            ...
            $(json_tally).each(function(idx, vote_data) {
                ...
                if (vote_data.is_most_voted) {
                    for (let i = 0; i < vote_data.voted_answers.length; i++) {
                        ...
                        let choice_object = $(question_object).find(`.multi-choice-letter[data-choice-letter='${letter}']`).parent();
    ...
    function show_community_votes(p1, p2) {
        ...
        if (!tally_json_container.length)
            return; // No JSON data.

        let vote_stats_json = JSON.parse(tally_json_container.text().trim());

        if (!vote_stats_json.length)
            return; // Empty votes object (no votes)
        ...
        for (var i = 0; i < vote_stats_json.length; i++) {
            ...
            if (bar_width < 15) {
                ...
                if (is_last_bar)
                    correlating_bar.attr("title", text);
            }
